...
const Learning = () => {
    ...
    return <Container maxWidth="sm" sx={{ padding: "1rem" }}>
        ...
        <Stack direction={"row"} spacing={"1rem"}>
            ...
            <Typography color={"red"} variant="h4"> :{words[count]?.meaning}</Typography>
            <Button sx={{ borderRadius: "50%" }}><VolumeUp /> </Button>
        </Stack>
        <Button sx={{ margin: "3rem 0" }} variant="contained" fullWidth onClick={count === words.length - 1 ? () => navigate("/quiz") : nextHandler}>{count === words.length - 1 ? "Test" : "Next"} </Button>
    </Container>
}
